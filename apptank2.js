...
// Πιθανά keys που μπορεί να έχεις στο Apps Script για τη Δεξαμενή 2
function getTank2Value(row) {
  const candidates = [
    "tank2", "tank_2", "tank2_level", "tank2Level", "Tank2",
    "deksameni2", "dexameni2", "reservoir2", "level2"
  ];
...
async function loadAndRender() {
  ...
  let processed = data.map(row => ({
    ...
    level: getTank2Value(row)
  }));

  processed = processed.filter(r => r.Data instanceof Date && !isNaN(r.Data.getTime()) && Number.isFinite(r.level));

  if (!processed.length) {
    if (statusEl) statusEl.textContent = "Δεν υπάρχουν έγκυρες τιμές δεξαμενής 2 (έλεγξε το όνομα πεδίου στο Apps Script).";
    return;
  }
